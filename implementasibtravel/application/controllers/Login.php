<?php

if (!defined('BASEPATH'))
	exit('No direct script access allowed');

class Login extends CI_Controller
{

	public $div = '<script type="text/javascript">alert("Selamat datang");</script>';
	function __construct()
	{
		parent::__construct();
		$this->load->model('Btravel_model');
	}

	public function index()
	{
		if($this->session->userdata('logined') && $this->session->userdata('logined') == true)
		{
			redirect('Btravel');
		}
		
		if(!$this->input->post())
		{
			$this->load->view('member/Home');
		}
		else
		{
			$cek_login = $this->Btravel_model->cek_login(
				$this->input->post('username'),
				md5($this->input->post('password'))
				);
			$cek_login_admin = $this->Btravel_model->cek_login_admin(
				$this->input->post('username'),
				$this->input->post('password')
				);
			if(!empty($cek_login))
			{
				$this->session->set_userdata('username',$cek_login->username_member);
				$this->session->set_userdata('logined',true);
				redirect('Btravel/home');
				$this->session->set_userdata('pesan', $div);
			}else if(!empty($cek_login_admin))
			{
				$this->session->set_userdata('usernameadmin',$cek_login_admin->username_admin);
				$this->session->set_userdata('logined',true);
				redirect('Btravel/home');
			}
			else
			{
				redirect("Btravel/home");
			}
		}
		
	} 
	
	public function logout()
	{
		$this->session->unset_userdata('logined');
		$this->session->unset_userdata('username');
		$this->session->unset_userdata('usernameadmin');
		
		redirect("Login");
	} 
}

?>
